...
namespace Astar_planner
{
  ...
  void AstarPlannerROS::initialize(const nav_msgs::OccupancyGrid::ConstPtr &map_)
  {
    ...
    if (!initialized_)
    {
      ...
      ROS_INFO("BAstar 全局规划器初始化准备,width = %d,height = %d,originX=%f,originY=%f",width,height,originX,originY);
      /*  
  ...
  std::vector<Eigen::Vector3d> AstarPlannerROS::makePlan(const vector<double> start_pos,const vector<double> target_pos)
  {
    ...
    start.pose.position.z = start_pos[2];
    start.header.frame_id = _frame_id;
    ...
    if (goal.header.frame_id != _frame_id)//判断目标点的父级坐标系是否是地图坐标系
    {
      ROS_ERROR("The goal pose passed to this planner must be in the %s frame.It is instead in the %s frame.",
                _frame_id.c_str(), goal.header.frame_id.c_str());
      // return ;
    }
    if (start.header.frame_id != _frame_id)//判断起始点的父级坐标系是否是地图坐标系
    {
